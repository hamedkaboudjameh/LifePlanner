...
import { Library, Plus, Star, BookOpen, Bot, Sparkles } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 text-purple-400 font-semibold text-xs mb-1">
            <Library className="w-4 h-4" /> Personal Library & Insights
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">
            Personal Bookshelf & AI Summarizer
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Track reading goals, extract key insights, and let Gemma generate structured book summaries.
          </p>
        </div>

        <button
          onClick={() => setShowAddModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold shadow-lg shadow-purple-600/25 transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {books.length === 0 ? (
          <div className="col-span-full glass-card p-12 text-center space-y-3">
            ...
            <h3 className="text-slate-200 font-bold text-lg">Your Bookshelf is Empty</h3>
            <p className="text-slate-400 text-xs max-w-sm mx-auto">
              Add books you are currently reading or have finished to track reviews and AI insights.
            </p>
            <button
              onClick={() => setShowAddModal(true)}
          ...
          books.map((book) => (
            <div key={book.id} className="glass-card glass-card-hover p-6 flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1 text-amber-400">
                    ...
                    <span className="text-xs font-bold">{book.rating || 5}/5</span>
                  </div>
                </div>
                ...
                {book.review?.summary && (
                  <div className="mt-4 p-3 rounded-lg bg-slate-900/80 border border-slate-800 text-xs text-slate-300 space-y-1">
                    <div className="flex items-center gap-1.5 text-purple-400 font-semibold text-[11px] mb-1">
                      <Sparkles className="w-3 h-3" /> AI Key Insights (Gemma)
                    </div>
              ...
              <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-500">
                <span>Status: <strong className="text-slate-300">{book.status}</strong></span>
              </div>
            </div>
          ))
        )}
          ...
          <div className="glass-card w-full max-w-lg p-6 space-y-5 border-purple-500/30">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-slate-100 text-lg flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-purple-400" /> Add Book to Library
              </h3>
              <button
            ...
            <form onSubmit={handleAddBook} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">
                  Personal Notes / Takeaways (Triggers AI Gemma Summarizer)
                </label>
                <textarea
                  value={userNotes}
                  onChange={(e) => setUserNotes(e.target.value)}
                  placeholder="Small 1% daily changes lead to massive compounding results..."
                  rows={3}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 focus:border-purple-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {loading ? "Generating Summary..." : "Save Book"}
                </button>
              </div>
            </form>
          </div>
